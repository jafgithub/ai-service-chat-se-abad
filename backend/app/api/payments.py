...
def start_checkout(payload: CheckoutRequest, db: Session = Depends(get_db)):
    ...
    if order.payment_method == "cod":
        # A cash order is created "confirmed", so the status check below already
        # stops this. Saying it explicitly means the protection does not quietly
        # depend on cash orders never being "pending", which a later change to
        # the order path could break without anyone noticing.
        raise HTTPException(
            status_code=409, detail="This order is being paid in cash on delivery.",
        )
    if order.status != "pending":
        # Already paid, or cancelled. Paying again would take money twice.
    ...
    try:
        session = provider.create_checkout(
            ...
            description=f"{settings.SHOP_NAME} order #{order.id}",
...
async def webhook(
    ...
    if event.status == "paid":
        payment.status = "paid"
        if job_service.confirm_order(db, order):
    ...
    # failed or cancelled: give the reserved stock back so it can sell again.
